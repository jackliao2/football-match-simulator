...
export async function generateMetadata({ params }: PageProps<"/es/[section]">): Promise<Metadata> { const { section } = await params; if (!valid(section)) notFound(); const copy = LOCALIZED_COPY.es; const item = section === "national-teams" ? copy.nations : section === "vs" ? copy.dreams : copy[section]; const path = localizedPath("es", `/${section}`); return { title: item.title, description: item.lead, alternates: { canonical: path, languages: languageAlternates(`/${section}`) }, openGraph: { title: item.title, description: item.lead, url: absoluteUrl(path), locale: "es_ES" } } }
