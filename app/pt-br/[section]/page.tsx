...
export async function generateMetadata({ params }: PageProps<"/pt-br/[section]">): Promise<Metadata> { const { section } = await params; if (!valid(section)) notFound(); const copy = LOCALIZED_COPY["pt-br"]; const item = section === "national-teams" ? copy.nations : section === "vs" ? copy.dreams : copy[section]; const path = localizedPath("pt-br", `/${section}`); return { title: item.title, description: item.lead, robots: { index: false, follow: true }, alternates: { canonical: path }, openGraph: { title: item.title, description: item.lead, url: absoluteUrl(path), locale: "pt_BR" } } }
