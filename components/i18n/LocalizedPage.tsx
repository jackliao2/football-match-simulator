...
import { FEATURED_MATCHUPS, HOMEPAGE_NATIONS, HOMEPAGE_TEAMS, todaysDebate, vsPath } from "@/data/matchups"
...
const SECTION_EDITORIAL: Record<Locale, Record<LocalizedSection, [string, string, string]>> = {
  es: {
    simulate: [
      "El motor escribe el Barça contra el Madrid. Un chatbot no.",
      "Esta página no adivina el próximo clásico. Sirve para sentar a dos plantillas con nombre y año — Barcelona 2010/11 contra Madrid 2016/17, Brasil 1970 contra España 2010 — y dejar que el motor escriba un marcador. La semilla cambia; el once no.",
      "Si buscabas cómo simular partidos de fútbol online, el flujo es ese: eliges temporadas, no abstracciones. La lectura de Barcelona 2010/11 lee las dos listas y las cien repeticiones después. No elige al ganador. Las notas son relativas a cada época: un 95 en 1970 describe dominio en 1970.",
    ],
    ...
    vs: ["Por qué Barça 2010/11 contra Madrid 2016/17 está aquí", "Cada duelo tiene una razón editorial: dos candidatos al mejor equipo de la historia, filosofías tácticas opuestas o generaciones que nunca pudieron enfrentarse. No llenamos la lista con partidos actuales solo porque sean recientes.", "La página de cada duelo incluye análisis escrito para esas dos selecciones, 400 simulaciones, marcadores frecuentes, goleadores, asistentes y los onces completos. Los porcentajes describen incertidumbre; no son cuotas ni consejos de apuestas."],
  },
  "pt-br": {
    simulate: ["O motor escreve o Barça contra o Madrid. Um chatbot não.", "Cada opção representa uma temporada específica, com técnico, formação, onze inicial e avaliações próprias. Barcelona 2010/11 contra Madrid 2016/17, ou Brasil 1970 contra Espanha 2010: o resultado combina essa identidade com uma semente aleatória. É uma noite possível, não a previsão de uma partida real.", "A leitura do Barcelona 2010/11 interpreta os dados depois da simulação; ela não escolhe o vencedor. As notas comparam o domínio de cada jogador em sua época e não transformam décadas diferentes em um teste físico moderno."],
    ...
    vs: ["Por que Barça 2010/11 contra Madrid 2016/17 está aqui", "Cada confronto tem uma razão editorial: dois candidatos a melhor time da história, filosofias táticas opostas ou gerações que nunca puderam se enfrentar. A lista não é preenchida com jogos atuais apenas porque são recentes.", "A página de cada duelo traz análise escrita para aquelas duas equipes, 400 simulações, placares frequentes, artilheiros, assistências e os onzes completos. As porcentagens descrevem incerteza; não são odds nem conselho de aposta."],
  },
}
...
export function LocalizedPage({ locale, section }: { locale: Locale; section?: LocalizedSection }) {
  ...
  if (section === "teams" || section === "national-teams") {
    ...
    const englishHref = isNations ? "/national-teams" : "/teams"
    return <div lang={locale === "pt-br" ? "pt-BR" : "es"} className="grid gap-6"><PageHeader kicker={isNations ? copy.nav.nations : copy.nav.teams} title={pageCopy.title} lead={pageCopy.lead} /><LocalizedEditorial locale={locale} section={section} /><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{items.map((team) => <TeamCard key={team.id} team={team} showSquad={false} />)}</div><div className="flex flex-wrap gap-3"><Link href={localizedPath(locale, "/simulate")} className="rail-btn rail-btn-primary rail-btn-inline">{copy.links.simulate}</Link><Link href={englishHref} className="font-mono text-sm text-gold hover:text-gold-2">{copy.links.englishCatalog} →</Link></div></div>
  }
  if (section === "vs") {
    return <div lang={locale === "pt-br" ? "pt-BR" : "es"} className="grid gap-6"><PageHeader kicker={copy.nav.dreams} title={copy.dreams.title} lead={copy.dreams.lead} /><LocalizedEditorial locale={locale} section="vs" /><div className="grid gap-2">{matches.map((match) => <MatchupRow key={match.href} {...match} />)}</div><Link href="/vs" className="font-mono text-sm text-gold hover:text-gold-2">{copy.links.englishCatalog} →</Link></div>
  }
  ...
  return (
    <div lang={locale === "pt-br" ? "pt-BR" : "es"} className="grid gap-5">
      ...
      <LocalizedSection title={copy.sections.clubs} href="/teams" link={copy.links.englishCatalog}><div className="grid gap-3 md:grid-cols-3">{clubs.slice(0, 3).map((team) => <TeamCard key={team.id} team={team} showSquad={false} />)}</div></LocalizedSection>
      <LocalizedSection title={copy.sections.nations} href="/national-teams" link={copy.links.englishCatalog}><div className="grid gap-3 md:grid-cols-3">{nations.slice(0, 3).map((team) => <TeamCard key={team.id} team={team} showSquad={false} />)}</div></LocalizedSection>
